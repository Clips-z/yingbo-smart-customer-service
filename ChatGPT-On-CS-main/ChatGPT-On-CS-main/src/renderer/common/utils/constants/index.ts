...
/**
 * 各大模型类型的默认 API 地址和推荐模型
 * 切换模型类型时会自动填充，用户只需填入 API Key 即可
 */
export const LLMDefaultConfig: Record<
  string,
  { baseURL: string; models: string[]; hint?: string }
> = {
  openai: {
  ...
  ernie: {
    ...
    hint: '百度文心：API 地址格式为 https://aip.baidubce.com/rpc/2.0/ai_custom/v1/wenxinworkshop/chat/{模型名}',
    models: ['ernie-4.0-8k', 'ernie-4.0-turbo-8k', 'ernie-speed-128k', 'ernie-lite-8k', 'ernie-3.5-8k'],
  },
  gemini: {
  ...
  minimax: {
    ...
    models: ['abab6.5-chat', 'abab6.5s-chat', 'abab6-chat', 'abab5.5-chat'],
  },
  qwen: {
    baseURL: 'https://dashscope.aliyuncs.com/compatible-mode/v1',
    models: ['qwen-turbo', 'qwen-plus', 'qwen-max', 'qwen-max-latest', 'qwen-long', 'qwen-vl-plus'],
  },
  spark: {
    baseURL: 'https://spark-api.xf-yun.com/v3.5',
    hint: '讯飞星火：需在控制台获取 API Key 和 App ID，地址末尾按版本填写 /v1、/v2、/v3.5 等',
  ...
  zhipu: {
    ...
    hint: '智谱 GLM：GLM-4-Flash 完全免费，注册 open.bigmodel.cn 获取 API Key 即可使用',
    models: ['glm-4-flash', 'glm-4', 'glm-4-plus', 'glm-4-long', 'glm-4v', 'glm-4-air', 'glm-4-airx'],
  },
  dify: {
    baseURL: 'https://api.dify.ai/v1',
    hint: 'Dify AI：请填入 Dify 平台提供的 API 地址和应用 API Key，模型名由 Dify 应用配置决定',
    models: ['gpt-4o', 'claude-3.5-sonnet', 'dify-auto'], // dify 模型名取决于应用配置
  },
  fastgpt: {
    baseURL: '',
    hint: 'FastGPT：请填入你的 FastGPT 部署地址 + /v1（如 https://your-fastgpt.com/api/v1）',
    models: ['', ], // 由 FastGPT 工作流配置决定
  },
};
