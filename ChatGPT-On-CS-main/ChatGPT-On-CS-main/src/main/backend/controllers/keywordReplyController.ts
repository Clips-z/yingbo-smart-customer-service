...
export interface KeywordCreateDTO {
  keyword: string;
  reply: string;
  platform_id?: string;
  fuzzy?: boolean;
...
export interface KeywordUpdateDTO {
  keyword?: string;
  reply?: string;
  platform_id?: string;
  fuzzy?: boolean;
...
export interface TransferKeywordCreateDTO {
  keyword: string;
  app_id?: string;
  fuzzy?: boolean;
  has_regular?: boolean;
...
export interface ReplaceKeywordCreateDTO {
  keyword: string;
  replace: string;
  app_id?: string;
  fuzzy?: boolean;
...
export class KeywordReplyController {
  ...
  async create(autoReplyData: KeywordCreateDTO) {
    return Keyword.create({ ...autoReplyData, mode: autoReplyData.mode ?? 'fuzzy' });
  }

  async update(id: number, autoReplyData: KeywordUpdateDTO) {
  ...
  async importExcel(path: string) {
    ...
    try {
      ...
      // 如果插入失败，用原始数据回滚（提取纯数据对象，排除 Sequelize Model 实例方法）
      const rollbackData = originalAutoReplies.map((r) => ({
        keyword: r.keyword,
        reply: r.reply,
        mode: r.mode,
        platform_id: r.platform_id,
        fuzzy: r.fuzzy,
        has_regular: r.has_regular,
      }));
      await Keyword.bulkCreate(rollbackData);
      throw error;
    }
  }
...
可使用的平台名称：${
  ...
  async createTransfer(autoReplyData: TransferKeywordCreateDTO) {
    return TransferKeyword.create({ ...autoReplyData });
  }

  async updateTransfer(id: number, autoReplyData: Partial<TransferKeywordCreateDTO>) {
  ...
  async importTransferExcel(path: string) {
    ...
    try {
      ...
      await TransferKeyword.bulkCreate(rollbackData);
      throw error;
    }
  ...
  async createReplace(autoReplyData: ReplaceKeywordCreateDTO) {
    return ReplaceKeyword.create({ ...autoReplyData });
  }

  async updateReplace(id: number, autoReplyData: Partial<ReplaceKeywordCreateDTO>) {
  ...
  async importReplaceExcel(path: string) {
    ...
    try {
      ...
      await ReplaceKeyword.bulkCreate(rollbackData);
      throw error;
    }
  }
  ...
  async getApps(): Promise<{ data: { id: string; name: string }[] } | null> {
    try {
      ...
      console.error(
        `获取平台信息失败: ${error instanceof Error ? error.message : String(error)}`,
      );
      return null;
    }
  }
