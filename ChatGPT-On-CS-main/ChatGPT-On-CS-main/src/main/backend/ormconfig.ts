...
import {
  Config,
  initConfig,
  checkAndAddFields as checkConfigFields,
...
import {
  Plugin,
  initPlugin,
  checkAndAddFields as checkPluginFields,
} from './entities/plugin';
import { initInstance } from './entities/instance';
import {
  Keyword,
  initKeyword,
  checkAndAddFields as checkKeywordFields,
...
const DB_DIR_ENV = process.env.DB_DIR ? process.env.DB_DIR : '';
const DOCUMENTS_DIR = DB_DIR_ENV
  ? path.resolve(DB_DIR_ENV) // 使用 resolve 确保路径是绝对的且格式正确
...
export const databaseReady = (async () => {
  try {
    ...
    await migrateEncryptedFields(sequelize);
  } catch (error) {
