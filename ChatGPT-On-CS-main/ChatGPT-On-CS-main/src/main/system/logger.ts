...
const writeConsole = (writer: (...args: unknown[]) => void, args: unknown[]) => {
  try {
    writer(...args);
  } catch (error: any) {
    if (error?.code !== 'EPIPE') {
...
console.log = (...args) => {
  writeConsole(originalConsoleLog, args);
  // if (!isDebug) return;
...
console.error = (...args) => {
  writeConsole(originalConsoleError, args);
  logToFile(
...
console.warn = (...args) => {
  writeConsole(originalConsoleWarn, args);
  logToFile(
