...
- RapidOCR 已改为长驻子进程：`scripts/qianniu-rapidocr-worker.py` 启动后只加载一次模型，Node 侧由 `qianniuOcrWorker.ts` 使用 JSON 行协议复用；首次冷启动超时为 120 秒，进程异常时会重建并回退到单次识别。
- 千牛伴随面板保留同一客户最近 3 段真实对话和历史 AI 草稿；已读、时间、价格、物流卡片、商品属性与转交提示会在后端统一过滤。同客户短暂 OCR 为空时沿用上次历史，不会跨客户复用。
- 采集健康接口现在包含 `warming/scanning/ready/retrying/idle` 阶段和最近耗时；`POST /api/v1/compat/qianniu/refresh` 只清理采集缓存并重试，不会点击千牛或发送消息。
